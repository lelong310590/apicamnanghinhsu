...
export default class PostsController {
  ...
  public async getTop5Post({request, response}: HttpContextContract){
    ...
    const post = await Post.query().preload('category').select(['id', 'name', 'description', 'status', 'author_id', 'image', 'views', 'created_at', 'updated_at', 'type_id']).limit(5).whereHas('category', (query) => {
      query.where('category_id',categoryId)
    }).orderBy('created_at',order).paginate(page, 5)
    if(post.length != 0){
      return response.status(200).json(
        new ResponseFormat(
          post,
